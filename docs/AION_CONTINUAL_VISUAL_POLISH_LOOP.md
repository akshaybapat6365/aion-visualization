...
1. Baseline scan
   ...
   - Run `npm run test:visual:control-tower` when a batch changes route visuals, route shell behavior, scene rendering, or visual QA expectations.
...
5. Verification
   ...
   - Use the Control Tower report in `test-results/control-tower/route-scores.md` as the generated route-score and screenshot index for the batch.
...
The Control Tower command writes generated evidence to `test-results/control-tower/`:

- `route-scores.md` for the route scoreboard, rubric scores, blockers, and next-batch candidates.
- `summary.json` for machine-readable route metrics.
- `latest/screenshots/*.png` for desktop, mobile, and narrow first-viewport screenshots.

These are review artifacts, not committed baselines. The current script targets the React/Vite canonical routes only. Older static visual-test documents and dashboards are historical unless regenerated by the current Vite route tooling.
