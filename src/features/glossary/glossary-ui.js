const GLOSSARY_PATH = 'data/glossary/canonical-glossary.json';
...
function createDeepLinks(term) {
  ...
  const chapterLinks = (term.relatedChapters || [])
    .map(ch => `<a href="chapter${ch}.html">Chapter ${ch}</a>`)
    .join(', ');
  const symbolLinks = (term.relatedSymbols || [])
    .map(symbol => `<a href="symbols.html?symbol=${encodeURIComponent(symbol)}">${symbol}</a>`)
    .join(', ');
...
function injectDrawerStyles() {
  ...
  style.textContent = `
    .glossary-drawer { position: fixed; top: 0; right: 0; width: min(420px, 92vw); height: 100vh; background: rgba(8,8,14,0.95); color: #fff; z-index: 10000; transform: translateX(100%); transition: transform 220ms ease; padding: 1rem; overflow: auto; border-left: 1px solid rgba(255,255,255,0.2); }
    .glossary-drawer.open { transform: translateX(0); }
    .glossary-trigger { margin-left: 0.5rem; }
    .glossary-term { padding: 0.8rem 0; border-bottom: 1px solid rgba(255,255,255,0.12); }
...
function mountGlossaryDrawer() {
  ...
  const renderTerms = (query = '') => {
    ...
    termList.innerHTML = state.glossary.terms
      ...
      .map((term) => {
        const chapterLinks = (term.relatedChapters || [])
          .map((chapter) => `<a href="chapter${chapter}.html">${chapter}</a>`).join(', ');
        const symbolLinks = (term.relatedSymbols || [])
          .map((symbol) => `<a href="symbols.html?symbol=${encodeURIComponent(symbol)}">${symbol}</a>`).join(', ');
        return `
